...
export class DetailSelectTool extends ToolWrapper {
  ...
  constructor() {
    ...
    this.tool.on({
      ...
      activate: () => console.log(paper.project),
      ...
      mousedrag: (event: paper.ToolEvent) => {
        ...
        for (const item of PaperUtil.getSelectedPaths()) {
          if (hitType === 'fill' || !item.segments) {
            ...
            if (PaperUtil.isCompoundPath(item.parent)) {
              continue;
            }
      ...
      mouseup: (event: paper.ToolEvent) => {
        if (doRectSelection && selectionRect) {
          PaperUtil.processRectangularSelection(event, selectionRect, true);
